...
import { objArrayToMap, composeContainers, reinterpolate, funcifyHandler } from '../utils';
...
export const useContainer = (containerMap, containerIds, components) => {
  const [Container, setContainer] = useState(composeContainers(containerMap, containerIds, components));
  ...
  useEffect(() => {
    ...
    if (isMounted.current) {
      setContainer(composeContainers(containerMap, containerIds, components));
    } else {
...
export const useInterpolation = (target = {}, data = {}, handlersKey = 'handlers') => {
  const deps = new Map();
